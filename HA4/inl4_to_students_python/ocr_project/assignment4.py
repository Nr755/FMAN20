...
def segment2feature(Si, target_size=128):
    """
...
# if __name__ == "__main__": 
#     # Keep placeholder (used for the first pass)
...
#     # -------- Pass 1: collect training data across all datasets --------
...
#             im2segment, segment2feature, feature2class, classification_data, datadir, mode
#         )
#         # Accumulate features/labels for training
#         trainX_list.append(allX)
#         trainY_list.append(allY)

#         print(ds + f' (collect): Hitrate = {hitrate*100:0.5}%')

#     # Stack into arrays for training
#     X_train = np.vstack(trainX_list)
#     Y_train = np.concatenate(trainY_list).ravel()

#     # -------- Train a real model --------
#     classification_data = class_train(X_train, Y_train, method="rf")  # or "svm"/"mlp"/"knn"

#     # -------- Pass 2: evaluate each dataset with the trained model --------
#     for ds in datasets:
#         datadir = os.path.join(thisdir, 'datasets', ds)
#         hitrate, confmat, allres, alljs, alljfg, allX, allY = benchmark_assignment3(
#             im2segment, segment2feature, feature2class, classification_data, datadir, mode
#         )
#         print(ds + f' (model): Hitrate = {hitrate*100:0.5}%')
