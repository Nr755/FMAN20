...
def computeFLIP(reference, test, PixelsPerDegree = 67):
    ...
    # --- Final error ---
    # Clamp to numerically safe ranges to avoid 0 ** (negative) = inf
    deltaEc = np.clip(deltaEc, 1e-6, None)        # avoid exact zeros
    expnt   = np.maximum(1.0 - deltaEf, 0.0)      # ensure non-negative exponent
    deltaE  = np.power(deltaEc, expnt)
